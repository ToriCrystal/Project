        ...
        <div class="col-lg-12">
            <div class="product__pagination">
                @if ($category->onFirstPage())
                    <span>&laquo;</span>
                @else
                    <a href="{{ $category->previousPageUrl() }}">&laquo;</a>
                @endif

                <a href="{{ $category->url(1) }}">1</a>
                @if ($category->currentPage() > 3)
                    <span>...</span>
                @endif

                @for ($i = max(2, $category->currentPage() - 2); $i <= min($category->lastPage() - 1, $category->currentPage() + 2); $i++)
                    @if ($i == $category->currentPage())
                        <a class="active" href="{{ $category->url($i) }}">{{ $i }}</a>
                    @else
                ...
                @if ($category->currentPage() < $category->lastPage() - 2)
                    <span>...</span>
                @endif
                <a href="{{ $category->url($category->lastPage()) }}">{{ $category->lastPage() }}</a>

                @if ($category->hasMorePages())
                    <a href="{{ $category->nextPageUrl() }}">&raquo;</a>
                @else
                    <span>&raquo;</span>
                @endif
            </div>
    ...
    @section('total')
        <div class="shop__product__option__left">
            <p>Hiện 1 – {{ $category->count() }} của {{ $totalCount }} kết quả!</p>
        </div>
    @endsection
